...
# --- Sidebar (MVP login + cert settings) ---
with st.sidebar:
    st.header("Login (MVP)")
...
# ----------------------------
# Reference tables (MODELED factors; not metered)
# ----------------------------
FUEL_EF_CO2 = {  # kg CO2 per kg fuel (typical)
    "HFO (Heavy Fuel Oil)": 3.114,
    "MGO/MDO (Marine Gas Oil/Diesel)": 3.206,
    "LNG": 2.750,  # CO2 only (methane slip is flagged separately)
    "Methanol (Green/Low-Carbon)": 1.375,  # placeholder pathway factor
    "Ammonia (Green)": 0.000,  # CO2 at exhaust near-zero; upstream later
}
...
base_daily_fuel_tonnes = (dwt / 10000) * 1.2  # rough scaling placeholder
...
carbon_price = {"None": 30, "Partial": 75, "High": 110}[eu_exposure]  # €/t proxy (MVP)
annual_cost_est_eur = annual_co2_tonnes * carbon_price * (
    0.0 if eu_exposure == "None" else 0.6 if eu_exposure == "Partial" else 1.0
)
...
if fuel_type in ["HFO (Heavy Fuel Oil)", "MGO/MDO (Marine Gas Oil/Diesel)"]:
    levers.append("Evaluate transition fuels (methanol / ammonia-ready strategy) for medium-term resilience.")
if fuel_type == "LNG":
    levers.append("Address methane slip risk roadmap (CH₄ focus): engine tuning + monitoring.")
if age >= 20:
    levers.append("Asset age risk is high; prepare charter/finance narrative with an improvement plan.")
...
# ----------------------------
# Certificate generator (FIXED)
# ----------------------------
def make_certificate_pdf(record: dict) -> bytes:
    pdf = FPDF()
    pdf.set_auto_page_break(auto=True, margin=12)
    pdf.add_page()  # ✅ Do NOT call output() before this
    ...
    pdf.multi_cell(
        ...
        "It is not a legal compliance guarantee. Estimates are derived from declared inputs and public factors."
    )
    ...
    pdf.multi_cell(
        ...
        f"- Simplified fuel-use model (placeholder). Future versions integrate telemetry/voyage evidence.\n"
        f"- CH₄ and N₂O are flagged as risk dimensions; quantified accounting needs metered pathways or verified MRV."
    )
...
st.caption("Next iterations: fleets, evidence uploads, GPS/voyage integration, insurer/bank views, and verified MRV connectors.")
